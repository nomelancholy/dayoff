...
- [x] 쿠폰 테이블/스키마 설계 (코드, 할인 타입·값, 유효기간 등) — `coupons`, `user_coupons` (server/db/schema/orders.ts)
- [x] 회원-쿠폰 연동 (발급/사용 내역) — NestJS Coupon 모듈 (관리자 CRUD·지급, 회원 내 쿠폰 목록·코드 검증 API), 프론트 `app/features/coupon` API·타입
- [x] 장바구니/결제 단계에서 쿠폰 적용 UI & 로직 — 장바구니 Order Summary에 쿠폰 코드 적용/해제 및 할인 반영
...
> Admin(`role = 'admin'`) 전용 페이지. 상품·유저·쿠폰·주문 등을 한곳에서 관리할 수 있는 백오피스.

- [x] **관리자 레이아웃·진입**: `/admin` 경로, Admin 전용 레이아웃(사이드 탭), 비로그인·일반 회원 접근 시 로그인/홈으로 리다이렉트
- [x] **상품 관리**: 상품 목록(테이블), 등록(`/shop/admin/new` 연동), 수정·삭제 + 관리자 페이지 내 카테고리 생성 — 기존 백엔드 API 활용
- [ ] **유저 관리**: 회원 목록(이메일, 이름, 역할, 가입일 등), 역할 변경(member ↔ admin), 필요 시 검색·필터
- [ ] **쿠폰 관리**: 쿠폰 목록, 생성·수정·삭제, 회원에게 쿠폰 지급 — Coupon API 활용
- [ ] **주문 관리** (선택): 주문 목록, 상태 변경(결제 완료·배송 중 등)
